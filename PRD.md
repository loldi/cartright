...
- **Interaction module (Telegram)** — inbound/outbound chat messages only, single private chat, single user. Not multi-tenant. (Pivoted from SMS/Twilio during go-live: US carrier A2P 10DLC / toll-free verification is disproportionate friction for a single-recipient personal agent, and a Telegram bot is free, instant, and registration-free. The transport sits behind a neutral `Messenger` interface, so the engine, scheduler, and conversation layer were unchanged by the swap.)
- **Review-order UI (web)** — a small server-rendered page (FastAPI + HTMX + Tailwind, matching existing stack defaults) that renders a `buildCart()` result as an itemized review with a single CTA that hands off to a real Walmart cart/checkout link. Never submits a purchase itself. **Superseded as the default alert path for items with real order history** (see "Alert message format" below, locked 2026-06-29): Telegram's own rich formatting now carries the itemized info and the cart-add link directly in the alert, so there's no separate page to tap through for the recurring/multi-item case. The page itself stays in the repo (still tested) but isn't linked from the current alert flow; revisit if a future mode (e.g. watch-only items with no purchase history, or much larger multi-item carts) needs a fuller breakdown than chat formatting can carry.
- **Adapters (not top-level modules, sit behind the `ShoppingEngine` seam):** an order-history reader (consumes the output of a private, excluded self-scrape utility — see Out of Scope/Further Notes), a Walmart catalog/pricing client (built against publicly documented walmart.io APIs only), and the messaging client (`Messenger`, Telegram in production). Production wires real adapters; tests wire fixture adapters satisfying the same interface.
- **Scheduler** — thin, not a deep module. Periodically calls `getReorderCandidates()` and, if a deal is found within a candidate's reorder window, hands off to the LLM layer to compose and send the message alert.
...
- Substitution is default-deny, not default-allow-with-opt-out: `buildCart()` may only place a substitute product on a cart line if the shopper has an explicit, recorded substitution grant for that item/category. No preference on file means no substitution — the absence of a "never substitute" flag is not implicit permission to substitute. This is stronger than a plain opt-out and must be enforced in `ShoppingEngine`/`build_cart`, not left to whatever the catalog adapter happens to return. Concretely: the grant is a positive `substitution_ok: true` attribute on the item's recorded preference (there is no `never_substitute` opt-out flag, because deny is already the default). When the catalog offers a substitute for an item with no such grant on file, `build_cart` drops the line entirely, treating it as unavailable exactly like an out-of-stock item, rather than keeping a flagged line. Dropping is the honest reading of the approve-then-handoff model and also guarantees an unapproved item_id can never leak into the walmart.com add-to-cart deep link.
...
**Alert message format (locked for MVP, 2026-06-29 — recurring/multi-item only; watch-only items deferred):**

- One single message, not two. Opens with a casual one-line greeting ("Hey there,"), then the item content, then the cadence/context detail (reorder interval, last-purchase date) as a plain final paragraph (no bold/italic) — all in one `sendMessage` call, with one inline button underneath.
- Item content: name bold and hyperlinked to its real Walmart product page, price bold, followed by a plain factual clause — `(you paid $X last time — save $Y)` — stated as fact only. No editorializing ("worth grabbing", "you're about due") anywhere in the message.
- One inline button: `Add to Walmart cart →` (multi-item: `Add all to Walmart cart →`), linking directly to `build_walmart_cart_url`'s real deep link. This is the only purchase-adjacent action — no review-page button.
- Multi-item: when more than one candidate's deal fires in the same scheduler cycle, send one combined message — each item on its own bulleted line in the same bold-name-linked/bold-price shape — with a single combined cart-add link covering all items and one shared detail paragraph, instead of one alert per item.
- Telegram auto-generates a large OpenGraph preview card for the first fetchable URL in a message — including a real product link, not just the cart-add link. Every send must set `disable_web_page_preview` (or `link_preview_options.is_disabled`), or a multi-item message balloons into a spammy photo card. Caught live: a placeholder item id used during message-format testing happened to resolve to a real Walmart product page and Telegram rendered its full preview card unprompted.
- Requires (not yet built): the `Walmart` adapter capturing walmart.io's real `productUrl` per item (not currently mapped in `_to_price`); `Messenger`/`TelegramMessenger.send_message` supporting `parse_mode`, `reply_markup`, and `disable_web_page_preview` (currently plain `text` only); the user's last-paid price for an item (from `OrderHistoryAdapter.get_orders()`) threaded into the alert path, since today's deal evaluation only has walmart.io's own `was_price`, not personal purchase history.
...
- The catalog/pricing adapter and the messaging adapter are tested via fakes that satisfy the same interface `ShoppingEngine` depends on. No test should hit a real Walmart endpoint or send a real message.
